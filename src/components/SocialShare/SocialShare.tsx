...
export function SocialShare({
  ...
  const handleShare = async (platform: string, shareUrl: string) => {
    ...
    // Close popup after sharing (for some platforms)
    if (popup) {
  ...
  return (
    <div className={`social-share ${variant} ${className}`}>
      ...
      {/* Copy link button */}
      <button
        className={`share-button copy-link ${copySuccess ? 'success' : ''}`}
      ...
      >
        ...
        {showLabels && <span>{copySuccess ? 'Copied!' : 'Copy Link'}</span>}
      </button>
...
export function FloatingShareButton(props: SocialShareProps) {
  ...
  return (
    <div className="floating-share-container">
      <motion.button
        className="floating-share-trigger"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="floating-share-menu"
            initial={{ opacity: 0, scale: 0.8, y: 10 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.8, y: 10 }}
            transition={{ duration: 0.2 }}
          >
            <SocialShare {...props} variant="vertical" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
